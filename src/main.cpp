 ...
 * Github: https://www.github.com/0x4248/fancy_uptime
...
std::string format_uptime(std::string uptime) {
    ...
    if (uptime.find("years") != string::npos) {
        uptime.replace(uptime.find("years"), 5,
                       normal.cyan + "years" + normal.reset + "\n");
    }
    if (uptime.find("months") != string::npos) {
        uptime.replace(uptime.find("months"), 6,
                       normal.cyan + "months" + normal.reset + "\n");
    }
    if (uptime.find("weeks") != string::npos) {
        uptime.replace(uptime.find("weeks"), 5,
                       normal.cyan + "weeks" + normal.reset + "\n");
    }
    if (uptime.find("days") != string::npos) {
        uptime.replace(uptime.find("days"), 4,
                       normal.cyan + "days" + normal.reset + "\n");
    }
    if (uptime.find("hours") != string::npos) {
        uptime.replace(uptime.find("hours"), 5,
                       normal.cyan + "hours" + normal.reset + "\n");
    }
    if (uptime.find("minutes") != string::npos) {
        uptime.replace(uptime.find("minutes"), 7,
                       normal.cyan + "minutes" + normal.reset + "\n");
    }
    if (uptime.find("seconds") != string::npos) {
        uptime.replace(uptime.find("seconds"), 6,
                       normal.cyan + "seconds" + normal.reset + "\n");
    }
    if (uptime.find("year") != string::npos) {
        uptime.replace(uptime.find("year"), 5,
                       normal.cyan + "years" + normal.reset + "\n");
    }
    if (uptime.find("month") != string::npos) {
        uptime.replace(uptime.find("month"), 6,
                       normal.cyan + "months" + normal.reset + "\n");
    }
    if (uptime.find("week") != string::npos) {
        uptime.replace(uptime.find("week"), 5,
                       normal.cyan + "weeks" + normal.reset + "\n");
    }
    if (uptime.find("day") != string::npos) {
        uptime.replace(uptime.find("day"), 4,
                       normal.cyan + "days" + normal.reset + "\n");
    }
    if (uptime.find("hour") != string::npos) {
        uptime.replace(uptime.find("hour"), 5,
                       normal.cyan + "hours" + normal.reset + "\n");
    }
    if (uptime.find("minute") != string::npos) {
        uptime.replace(uptime.find("minute"), 7,
                       normal.cyan + "minutes" + normal.reset + "\n");
    }
    if (uptime.find("second") != string::npos) {
        uptime.replace(uptime.find("second"), 6,
                       normal.cyan + "seconds" + normal.reset + "\n");
    }
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) {
            cout << bold.green << "Fancy uptime for linux systems" << bold.reset
                 << endl;
            cout << bold.green << "Usage: " << bold.reset << "uptime [OPTION]"
                 << endl;
            cout << bold.green << "Options:" << bold.reset << endl;
            cout << bold.green << "  -h, --help" << bold.reset
                 << "     Show this help" << endl;
            cout << bold.green << "  -v, --version" << bold.reset
                 << "  Show version" << endl;
            cout << bold.green << "  -nc, --no-colour" << bold.reset
                 << "  No colour" << endl;
            return 0;
